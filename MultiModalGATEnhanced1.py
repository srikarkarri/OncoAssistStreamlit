...
# ============================================================================
# CLASS DEFINITIONS ONLY - NO EXECUTABLE CODE
# ============================================================================
...
class MultiModalGAT(nn.Module):
    ...
    # ------- inside predict_proba() for ndarray input -----------------
    def predict_proba(self, data, temperature=1.0, clip_min=1e-7):
        """
        Return calibrated class probabilities for simple feature arrays.
        """
        self.eval()
        with torch.no_grad():
            if not isinstance(data, np.ndarray):
                raise ValueError("For graph objects call the regular forward pass")

            # 1. Forward the batch through the linear classifier only
            x = torch.tensor(data, dtype=torch.float32, device=next(self.parameters()).device)
            logits = self.pam50_classifier(x)                 # shape [B, 4]

            # 2. Temperature-scale logits (T<1 → sharper, T>1 → flatter)
            ...
            # 4. Numerical safety
            probs = np.clip(probs, clip_min, 1.0)
...
def main():
    ...
    # X_val          → numpy or torch tensor of shape [N, input_dim]
    # y_val          → 1-D numpy array of integer class labels
    X_val_t = torch.tensor(X_val, dtype=torch.float32).to(device)

    # raw logits from the classifier head
    with torch.no_grad():
        logits_val = model.pam50_classifier(X_val_t)          # shape [N, 4]

    # grid-search for the scalar temperature T
    T_grid   = np.linspace(0.3, 3.0, 27)
    best_T   = 1.0
    best_nll = 1e9

    for T in T_grid:
        p = torch.softmax(logits_val / T, dim=1).cpu().numpy()
        nll = log_loss(y_val, p)                              # negative-log-likelihood
        if nll < best_nll:
            best_nll, best_T = nll, T

    print(f"✅ Optimal temperature found: {best_T:.3f}  (valid NLL {best_nll:.4f})")

    model.temperature = float(best_T)          # plain Python float is easiest to pickle
